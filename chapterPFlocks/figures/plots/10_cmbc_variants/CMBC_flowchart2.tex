...
\begin{document}
    \begin{tikzpicture}[node distance = 2cm, auto]
        ...
        % Place nodes
        \node [block] (cliques) {Compute cliques};
        \node [cloud, left of=cliques, node distance=3cm] (input) {$\mathcal{P}oints$, $\varepsilon$, $\mu$};
        ...
        \node [block, below left of=decide, node distance=3cm, text width=8em] (traditional) {Apply pruning procedure on cliques not enclosed};
        \node [cloud, below of=traditional, node distance=2cm] (maximals) {$\mathcal{M}aximals\_disks$};

        % Draw edges
        \path [line,dashed] (input) -- (cliques);
