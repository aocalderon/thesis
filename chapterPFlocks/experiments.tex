...
Instead, we explored graph-oriented clustering, focusing on the concept of \textit{maximal cliques}. In an undirected graph, a \textit{maximal clique} is a subset of
...
potential disk centers, identifying candidate disks, and pruning them, as outlined in Figure \ref{fig:MF_flowchart}. Figure \ref{app:cmbc_flowchar} illustrates the steps described above.  We have termed this approach as CMBC.
...
Figure \ref{fig:cmbc_500_psi} compares the performance of the two CMBC variants with the time taken by PSI for the same stage. Interestingly, neither variant achieves a reduction in execution time. Upon closer analysis, it becomes evident that while identifying the cliques and their MBCs is relatively fast, only a small fraction of the cliques qualify as maximal disks. Consequently, the overhead associated with processing the remaining cliques outweighs the benefits, making the original PSI approach more efficient.
...
We attempted to increase the density within the partition to determine whether a higher number of cliques would qualify as maximal disks and improve performance. To achieve this, we measured the density of points and pairs in the current partition and conducted simulations to introduce additional trajectory points within the same space. The number of points in the partition was incrementally increased from 500 to 1K, 2K, 4K, and 6K to test varying densities. For each simulation, we recorded the execution time required to identify maximal disks, increasing the value of $\varepsilon$ from 10 to 20 meters. Additionally, we tracked the number of pairs generated for each value of $\varepsilon$ as a measure of density. Figure \ref{fig:cmbc_pairs} presents the performance of CMBC (using the COLLECT variant) compared to PSI as the number of points and pair density increased.
...
\begin{figure}
    ...
    \caption{Execution time of the two variants of CMBC approach compared to standard PSI in the partition 187 with $aprox$2K points present ($\varepsilon=20m$).}\label{fig:cmbc_2K_psi}
\end{figure}
